...
import com.inke.library.helper.PermissionHelper;
...
public class PermissionManager {
     ...
     * @return 如果所有的权限都已被授权返回true, 反之哪怕有一个没有被授权通过返回false
     */
    ...
    private static void notifyHasPermissions(Activity activity, int requestCode, String[] perms) {
        ...
        for (int i = 0; i < perms.length; i++) {
            String perm = perms[i];
            grantResults[i] = ContextCompat.checkSelfPermission(activity, perm);
        }
    ...
    public static void onRequestPermissionResult(int requestCode, String[] permissions, int[] grantResults, Activity activity) {
        ...
        //如果授权全部通过，才执行注解方法，哪怕多个权限中一个被拒绝也不执行方法
     ...
     */
    public static boolean somePermissionPermanentlyDenied(Activity activity, List<String> deniedPermission) {
        return PermissionHelper.newInstance(activity).somePermissionPermanentlyDenied(deniedPermission);
    }
}
